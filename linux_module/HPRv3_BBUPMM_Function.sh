...
PMM_Update_Process()
{
    ...
    if [ ${PMMVendor^^} == "PANASONIC" ] && [ ${PMMType} == "ORV3_HPR_PMM_BBU" ];then
    ...
    elif ([ ${PMMVendor^^} == "DELTA" ] || ${PMMVendor^^} == "Delta" ]) && [ ${PMMType} == "ORV3_HPR_PMM_BBU" ] ;then
...
{
    record_time "HPRv3_WG400_BBUPMM" start "Wedge400_GPIO_Test;NA" "6" "NA" "${RackSN}"
	...
	if [ ${GPIONum} -ne 6 ];then
        ...
        record_time "HPRv3_WG400_BBUPMM" end "Wedge400_GPIO_Test;NA" "6" "FAIL" "${RackSN}"
        record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "FAIL" "${RackSN}"
	    update_status "$SN" "$folder" "$index" "$testitem" "GPIO Test" 3
        return 1
    else
        show_pass_msg "Rack Module Information Test -- Wedge400 GPIO Test"
        record_time "HPRv3_WG400_BBUPMM" end "Wedge400_GPIO_Test;NA" "6" "PASS" "${RackSN}"
...
Normal_Wedge400_Gen_Whole_Log()
{
	...
	update_status "$SN" "$folder" "$index" "$testitem" "Get Whole Log Test" 3
        ...
        record_time "HPRv3_WG400_BBUPMM" end "Wedge400_Whole_Log;NA" "1000" "PASS" "${RackSN}"
    fi    
}
...
Normal_HPRv3_PMM_Funtional_test()
{
	...
	for address in ${HPR_PMM_BBU_AddressArr[@]};do
	    ...
	    if [[ " ${BBU_PMM_STDARR[@]} " =~ " $address " ]]; then
            ...
            echo "${address} is not in BBU_PMM_STDARR array, stop the test and check the cable connection"
            record_time "HPRv3_WG400_BBUPMM" end "BBU_PMM_Check;${address}" "${address}" "FAIL" "${RackSN}"
            record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "FAIL" "${RackSN}"
        ...
        for i in "${!PMMInfo[@]}"; do
            ...
            record_time "HPRv3_WG400_BBUPMM" start "BBU_${address}_PMM_Information_Check;${PMMInfo[$i]}" "${PMM_Array[$i]}" "NA" "${RackSN}"
        ...
        for BBU in "${BBUAlert[@]}"; do
            record_time "HPRv3_WG400_BBUPMM" start "BBU_${address}_PMM_Alert_Check;${address}" "[0]" "NA" "${RackSN}"	
            logResultCheck "LogCheck" "${BBU}" "1;[0]" "${BBUPMMLog}"
            if [ ${PIPESTATUS[0]} -ne 0 ];then
                show_fail_msg "Rack Module Test -- BBU ${BBU} PMM Alert Test"
                record_time "HPRv3_WG400_BBUPMM" end "BBU_${address}_PMM_Alert_Check;${address}" "[0]" "FAIL" "${RackSN}"
                record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "FAIL" "${RackSN}"
		        update_status "$SN" "$folder" "$index" "$testitem" "BBU PMM Alert Test" 3
                return 1
            else
                show_pass_msg "Rack Module Test -- BBU ${BBU} PMM Alert Test"
                record_time "HPRv3_WG400_BBUPMM" end "BBU_${address}_PMM_Alert_Check;${address}" "[0]" "PASS" "${RackSN}"
...
record_time "HPRv3_WG400_BBUPMM" initial "" "" "" "$RackSN" 

Normal_HPRv3_GPIO_test | tee -a ${LOGFILE}
if [ "${PIPESTATUS[0]}" -ne "0" ];then
	show_error_msg "HPRv3 PMM Functional Test" | tee -a ${LOGFILE}
    show_fail | tee -a ${LOGFILE}    
    finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_${switch_index}_${wedge400SN}_BBUPMMFUNC_FAIL_${startTime}.log
    record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "FAIL" "${RackSN}"
   	record_time "HPRv3_WG400_BBUPMM" show "" "" "" "${RackSN}" | tee -a ${LOGFILE}
...
if [ "${PIPESTATUS[0]}" -ne "0" ];then
    show_error_msg "HPRv3 BBU PMM Functional Test" | tee -a ${LOGFILE}
    show_fail | tee -a ${LOGFILE}    
    finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_${switch_index}_${wedge400SN}_BBUPMMFUNC_FAIL_${startTime}.log
    record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "FAIL" "${RackSN}"
    record_time "HPRv3_WG400_BBUPMM" show "" "" "" "${RackSN}" | tee -a ${LOGFILE}
...
echo "HPRv3 BBU PMM Functional Test" | tee -a ${LOGFILE}
show_pass | tee -a ${LOGFILE}
finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_${switch_index}_${wedge400SN}_BBUPMMFUNC_PASS_${startTime}.log
record_time "HPRv3_WG400_BBUPMM" total "HPRv3_WG400_BBUPMM;NA" "NA" "PASS" "${RackSN}"
record_time "HPRv3_WG400_BBUPMM" show "" "" "" "${RackSN}" | tee -a ${LOGFILE}
#cat ${LOGPATH}/${RackSN}/summary_table_${station}.conf | tee -a ${LOGFILE}
cat ${LOGFILE} > ${LOGPATH}/${RackSN}/${finalLog}
cp ${LOGPATH}/${RackSN}/${finalLog} /log/hprv3 > /dev/null
