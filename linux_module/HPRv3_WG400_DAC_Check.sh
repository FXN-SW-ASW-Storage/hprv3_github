...
while getopts s:i: OPT; do
    case "${OPT}" in
        "s")
            ...
            sw_ip=$(cat ${LOGPATH}/${RackSN}/RUSW/${index}/mac_ip.txt)
            sw_sn=$(cat ${LOGPATH}/${RackSN}/RUSW/${index}/serialnumber.txt)
            ;;
        "i")
            index=${OPTARG}
            if [ -z "${index}" ];then
...
check_DAC_status()
{
	PortResultLog=${LOGFOLDER}/RUSW/$index/DAC_Check_portResult.log
	...
	#record_time "$session" total "$session;All" "NA" "PASS" "${RackSN}"
        ...
        cat ${PortResultLog} | grep "ce15\|ce19\|ce23"
...
diagnosticLog=${LOGFOLDER}/RUSW/$index/dac_diagnostic_wg400.txt
LOGFILE=${LOGFOLDER}/RUSW/$index/daclog.txt
...
if [ "${PIPESTATUS[0]}" -ne "0" ];then
    ...
    finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_WG400_${index}_${sw_sn}_DACCHECK_FAIL_${startTime}.log
    cat ${LOGFILE} > ${LOGPATH}/${RackSN}/RUSW/$index/${finalLog}
    cp ${LOGPATH}/${RackSN}/RUSW/$index/${finalLog} /log/hprv3 > /dev/null
    exit 1
fi
...
finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_WG400_${index}_${sw_sn}_DACCHECK_PASS_${startTime}.log
cat ${LOGFILE} > ${LOGPATH}/${RackSN}/RUSW/$index/${finalLog}
cp ${LOGPATH}/${RackSN}/RUSW/$index/${finalLog} /log/hprv3 > /dev/null
