...
main()
{
    ...
    if [ "${res}" -ne "0" ];then
        show_fail_msg "Ping all switches ${name1} IP"
        return 1
    elif [ ! "${chk_pass_count}" = "${pass}" ];then
	    show_fail_msg "Only ping ${pass} switches pass,need ${chk_pass_count} switches pass."
    else
        show_pass_msg "Ping all switches ${name1} IP"
    fi
}
