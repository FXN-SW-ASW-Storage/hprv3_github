...
function print_help() {
    ...
    [OPTION]:
        ...
        m : disable SFC.
    Example: $(basename $0) -s JDYD1234567890 [ -i 1 ]
EOF
}
...
run_command(){
    ...
    ./$m.sh -i ${index} -s $SN | tee -a ${LOGFILE}
...
{  
    ...
    if [ ! -d "${LOGFOLDER}/MP3_MAIN/PASS" ] || [ ! -d "${LOGFOLDER}/MP3_MAIN/FAIL" ] ;then
        mkdir -p "${LOGFOLDER}/MP3_MAIN/PASS"
        mkdir -p "${LOGFOLDER}/MP3_MAIN/FAIL"
    fi
    item=${2}
    ...
    log_name="${rack_sn}"_"${platform}"_"${PROJECT}"-${index}_${sw_sn}_MP3_"${Flag}"_${1}_"${time}".log
    file_name="${LOGFOLDER}/MP3_MAIN/${1}/${log_name}"
    echo "****************************************************" >${file_name}
    ...
    echo "mp2_main_flow Log file : ${file_name}"
#----------------------------- remark by Otis -----------------------------#
#    umount /log/diag
...
get_test_item(){
    test_item="chk_fsw_ip run_mp3_test HPRv3_MP3_BMC_Function HPRv3_MP3_PRBS31_PVT HPRv3_MP3_SNAKETRAFFIC_PVT HPRv3_MP3_INSERTIONLOSS_PVT"
    #test_item="chk_fsw_ip run_mp3_test HPRv3_MP3BMC_Function HPRv3_MP3_PRBS31_PVT HPRv3_MP3_SNAKETRAFFIC_PVT HPRv3_MP3_INSERTIONLOSS_PVT"
    #test_item="chk_fsw_ip run_mp3_test HPRv3_MP3BMC_Function HPRv3_MP3_PRBS31 HPRv3_MP3_SNAKETRAFFIC"
    #test_item="chk_fsw_ip mp2_test mp2_prbs31"
} 
...
#fi
	...
	get_test_item
	run_command "${test_item}" "$index" "$SN" 
    if [[ "$?" -eq "0" ]]; then                
        upload_log PASS
        show_pass
		exit 0
