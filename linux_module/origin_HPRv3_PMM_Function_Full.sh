...
Normal_Wedge400_Gen_Whole_Log()
{
    ...
    if [ ${rackmonlogCount} -lt 900 ];then
        ...
        record_time "HPRv3_WG400_PMM" end "Wedge400_Whole_Log;NA" "1000" "FAIL" "${RackSN}"
        record_time "HPRv3_WG400_PMM" total "HPRv3_WG400_PMM;NA" "NA" "FAIL" "${RackSN}"
	...
	update_status "$SN" "$folder" "$index" "$testitem" "Get Whole Log Test" 2
        record_time "HPRv3_WG400_PMM" end "Wedge400_Whole_Log;NA" "1000" "PASS" "${RackSN}"
    fi    
...
Normal_HPRv3_PMM_Funtional_test()
{
	...
	declare -a DELTA_BBUPMM_Data_Array=("03-100037" "ECD90000111" "^[0-9]{2}/[0-9]{4}$" "M2HPDET" "PR" "1140")
	...
	for address in ${HPR_PMM_PSU_AddressArr[@]};do
		if echo "${PSU_PMM_STDARR[@]}" | grep -wq "$address"; then
	...
	update_status "$SN" "$folder" "$index" "$testitem" "PSU PMM Alert Test" 1
        ...
        for PSU in "${PSUAlert[@]}"; do
           #record_time "HPRv3_WG400_PMM" start "PSU_${address}_PMM_Alert_Check;${PSU}" "[1]" "NA" "${RackSN}"
          ...
          logResultCheck "LogCheck" "${PSU}" "1;[0]" "${PSUPMMLog}"
           if [ ${PIPESTATUS[0]} -ne 0 ];then
               ...
               #record_time "HPRv3_WG400_PMM" end "PSU_${address}_PMM_Alert_Check;${PSU}" "[1]" "FAIL" "${RackSN}"
	...
	if echo "${BBU_PMM_STDARR[@]}" | grep -wq "$address"; then
...
if [ "${PIPESTATUS[0]}" -ne "0" ];then
	show_error_msg "HPRv3 PMM Functional Test" | tee -a ${LOGFILE}
    	...
    	finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_${switch_index}_${wedge400SN}_PMMFUNC_FAIL_${startTime}.log
        record_time "HPRv3_WG400_PMM" total "HPRv3_WG400_PMM;NA" "NA" "FAIL" "${RackSN}"
...
finalLog=${RackIPN}_${RackSN}_${RackAsset}_HPRv3_${switch_index}_${wedge400SN}_PMMFUNC_PASS_${startTime}.log
