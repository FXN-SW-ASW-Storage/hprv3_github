...
MP3_Function_Diagnostic_test()
{
    ...
    Resultstatus=$(cat ${diagnosticLog} | grep "total_funcs" | awk '{print$NF}' |tail -n1 | tr -d '\r\n' | tr -d '[' | tr -d ']')
    ...
    MCBFRU_DATA=("0xFBFB" "0x[5-6]" "MINIPACK3_MCB" "" "" "132100094" "131100034" "R3214G000204" "A[0-9]{12}" "[2-4]|[1-4]" "[2-4]|[1-4]" "0" "" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "GCE|ZFP" "CTH" "MCB" "0x[0-9a-z]+")
    ...
    SMBFRU_DATA=("0xFBFB" "0x5|0x6" "MINIPACK3_SMB" "20100187" "" "132100093" "131100033" "R3214G000104" "A[0-9]{12}" "[2-4]|[1-4]" "[2-4]|[1-4]" "20" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "ZFP|WUS" "CTH" "SMB" "0x[0-9a-z]+")
    #SMBFRU2_DATA=("0xFBFB" "0x5" "MINIPACK3_SMB" "20100187" "" "132100093" "131100033" "R3214G000104" "A[0-9]{12}" "3" "4" "20" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|i[01])" "WUS" "CTH" "SMB" "0x[0-9a-z]")

    SCMFRU_DATA=("0xFBFB" "0x[5-6]" "MINIPACK3_SCM" "20100188" "" "132100049" "131100035" "R3214G000304" "A[0-9]{12}" "[2-4]|[1-4]" "[2-4]|[1-4]" "10" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "ZFP" "CTH" "SCM" "[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}" "1" "0x[0-9a-z]+")
    SCMFRU2_DATA=("0xFBFB" "0x[5-6]" "MINIPACK3_SCM" "20100188" "" "132100049" "131100035" "R3214G000304" "A[0-9]{12}" "[2-4]" "[2-4]" "10" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "GCE" "CTH" "SCM" "[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}" "1" "0x[0-9a-z]+")
    ...
    for i in "${!FRUInfo[@]}"; do
        ...
        if [ "${FRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ]  || [ "${FRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${FRUInfo[$i]}" == "CRC16" ] || [ "${FRUInfo[$i]}" == "Format Version" ] || [ "${FRUInfo[$i]}" == "Product Production State" ] || [ "${FRUInfo[$i]}" == "Product Version" ] || [ "${FRUInfo[$i]}" == "PCB Manufacturer" ];then
    ...
    for i in "${!FRUInfo[@]}"; do
        ...
        if [ "${FRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ] || [ "${FRUInfo[$i]}" == "Product Serial Number" ] || [ "${FRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${FRUInfo[$i]}" == "CRC16" ] || [ "${FRUInfo[$i]}" == "Format Version" ] || [ "${FRUInfo[$i]}" == "Product Production State" ]|| [ "${FRUInfo[$i]}" == "Product Version" ] || [ "${FRUInfo[$i]}" == "PCB Manufacturer" ];then
            logResultCheck "CheckEGREPFormat" "${FRUInfo[$i]}" "NF;${FRU_FinalArr[$i]}" "${diagnosticLog}"
            if [ ${PIPESTATUS[0]} -ne 0 ];then
		echo "Select another FRU data to check again"
