...
MP3_Function_Diagnostic_test()
{
    ...
    MCBFRU_DATA=("0xFBFB" "0x6" "MINIPACK3_MCB" "" "" "132100094" "131100034" "R3214G000204" "A[0-9]{12}" "3" "4" "0" "" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "ZFP" "CTH" "MCB" "0x[0-9a-z]+")
    ...
    SMBFRU_DATA=("0xFBFB" "0x6" "MINIPACK3_SMB" "20100187" "" "132100093" "131100033" "R3214G000104" "A[0-9]{12}" "3" "4" "20" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "WUS" "CTH" "SMB" "0x[0-9a-z]+")
    ...
    SCMFRU_DATA=("0xFBFB" "0x6" "MINIPACK3_SCM" "20100188" "" "132100049" "131100035" "R3214G000304" "A[0-9]{12}" "3" "4" "10" "M[0-9]{12}" "CLS" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "ZFP" "CTH" "SCM" "[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}" "1" "0x[0-9a-z]+")
    ...
    for i in "${!FRUInfo[@]}"; do
        ...
        if [ "${FRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ]  || [ "${FRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${FRUInfo[$i]}" == "CRC16" ];then
    ...
    for i in "${!FRUInfo[@]}"; do
        ...
        if [ "${FRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ] || [ "${FRUInfo[$i]}" == "Product Serial Number" ] || [ "${FRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${FRUInfo[$i]}" == "CRC16" ];then
            logResultCheck "CheckEGREPFormat" "${FRUInfo[$i]}" "NF;${FRU_FinalArr[$i]}" "${diagnosticLog}"
            if [ ${PIPESTATUS[0]} -ne 0 ];then
		echo "Select another FRU data to check again"
    ...
    for i in "${!SCMFRUInfo[@]}"; do
        ...
        if [ "${SCMFRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ] || [ "${SCMFRUInfo[$i]}" == "Product Serial Number" ] || [ "${SCMFRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${SCMFRUInfo[$i]}" == "X86 CPU MAC Base" ] || [ "${SCMFRUInfo[$i]}" == "X86 CPU MAC Address Size" ] || [ "${SCMFRUInfo[$i]}" == "CRC16" ];then
    ...
    PSUFRU_DATA=("0xFBFB" "0x6" "DC3K12V_M_L" "00000000" "03001202" "000000000000" "000000000000" "DD-2302-1L" "[0-9]{1}[A-Z]{2}[0-9]+" "3" "1" "0" "[0-9]{1}[A-Z]{2}[0-9A-Z]+" "Liteon" "202[4-5](0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])" "LaiHe" "Liteon" "PSU" "0x[0-9a-z]+")
    ...
    for i in "${!PSUFRUInfo[@]}"; do
        ...
        if [ "${PSUFRUInfo[$i]}" == "ODM/JDM PCBA Serial Number" ] || [ "${PSUFRUInfo[$i]}" == "Product Serial Number" ] || [ "${PSUFRUInfo[$i]}" == "System Manufacturing Date" ] || [ "${PSUFRUInfo[$i]}" == "CRC16" ];then
...
if [ "${PIPESTATUS[0]}" -ne "0" ];then
   ...
   finalLog=${RackIPN}_${RackSN}_${RackAsset}_MP3_${index}_Function_FAIL_${startTime}.log
...
finalLog=${RackIPN}_${RackSN}_${RackAsset}_MP3_${index}_Function_PASS_$startTime.log
