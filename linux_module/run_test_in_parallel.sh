...
main()
{
    ...
    do
        fsw_session_name="${SN}-FSW-${loc_id}"
        chk=`tmux ls | grep -c "${fsw_session_name}"`
        if [ "${chk}" -ne "0" ]; then
            echo "${fsw_session_name} is already alive.Kill the session"
            tmux kill-session -t ${fsw_session_name}
            sleep 3
        fi
        if ! tmux new -s ${fsw_session_name}; then
            #if [ -z "${res}" ];then
            show_fail_message "Start tmux session ${fsw_session_name}"
            echo "Please make sure if install tmux well first."
        else
            show_pass_message "Start tmux session ${fsw_session_name}"
        fi
        tmux set -g history-limit 3600 >/dev/null
        tmux send-keys -t ${fsw_session_name} "export SN=${SN}" ENTER
        tmux send-keys -t ${fsw_session_name} "source ../commonlib" ENTER
        tmux send-keys -t ${fsw_session_name} "./${script_name} -s ${SN} -i ${loc_id}" ENTER
        echo "Run \" ./${script_name} -s ${SN} -i ${loc_id} \" on ${fsw_session_name}"
        echo "tmux a -t ${fsw_session_name}"
        sleep 1
    done
