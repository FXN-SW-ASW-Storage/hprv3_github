...
Normal_Function_Diagnostic_test()
{
    ...
    SMBFRU_DATA=("3" "WEDGE400-48VDC-F" "20-002948" "00-000000" "132-000137-02" "131-000103-02" "R1149G900102" "GE[0-9]{11}" "4" "4|5" "0" "FH[0-9]{11}" "[0-9]{8}" "CLS" "^[0-1][0-9]-[0-3][0-9]-[0-9]{2}$" "WUS|ISU" "CTH" "[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}" "[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}" "143" "SMB" "0x[0-9a-z]{2}|0x[0-9a-z]{1}")
    ...
    record_time "HPRv3_Wedge400" start "Sensor Test;NA" "OK" "NA" "${RackSN}"
    ...
    record_time "HPRv3_Wedge400" start "SSD FW Test;NA" "OK" "NA" "${RackSN}"
    ...
    SSDFW=("11130120" "AGFE5103" "10105120" "41021C20" "APFE5102" "51022A20")	
     
    Serial_number_log=$(cat ${diagnosticLog} | grep 'sn ' | awk -F ': ' '{print $2}' | tr -d ' ' | tr -d '\r\n')
    #Serial_number_log=$(grep "sn" "${diagnosticLog}" | awk -F ': ' '{print $2}' | tr -d ' ' | tr -d '\r\n'|| echo "")
    Serial_number_ini=$(cat ${INIPATH}/Wedge400_SSDFW.ini | grep 'sn ' | awk -F ': ' '{print $2}' | tr -d ' ' | tr -d '\r\n')
    ...
    FW_log=$(cat ${diagnosticLog} | grep 'fr ' | awk -F ': ' '{print $2}' | tr -d ' ' | tr -d '\r\n') 
    FW_flg=1
    for STDFW in ${SSDFW[@]}
    do
        if [ ${FW_log} == ${STDFW} ];then
            echo "Current SSD FW is match the $STDFW"
            FW_flg=0
        else
            echo "Current SSD FW still don't get the match,continue to check"
        fi
    done
    ...
    if [ "$FW_flg" == 1 ]; then
        ...
        show_fail_msg "Rack Module Test -- SSD FW Test -- FW Error"                         
        ...
        record_time "HPRv3_Wedge400" end "SSD FW Test;NA" "${FW_log}" "PASS" "${RackSN}" 
    ...
    SW_DATA=("3.7.4|3.7.0" "2.1.4" "7.4.1708" "4.19.17" "4.76" "XG1_3B15|XG1_3A12" "1.12" "1.12" "4.1" "4.6" "3.25")
    #SW_DATA2=("3.7.0" "2.1.4" "7.4.1708" "4.19.17" "4.76" "XG1_3B15" "1.12" "1.12" "4.1" "4.6" "3.25")
    ...
    for i in "${!SWInfo[@]}"; do
        record_time "HPRv3_Wedge400" start "Product_Information_Check;${SWInfo[$i]}" "${FRU_FinalArr[$i]}" "NA" "${RackSN}"
        if [ $(echo ${SWInfo[$i]} | tr -d ' ') == "DiagVersion" ] || [ $(echo ${SWInfo[$i]} | tr -d ' ') == "BIOSVersion" ];then
            logResultCheck "CheckEGREPFormat" "${SWInfo[$i]}" "NF;${FRU_FinalArr[$i]}" "${diagnosticLog}"
            if [ ${PIPESTATUS[0]} -ne 0 ];then
