...
Normal_Function_Diagnostic_test()
{
    ...
    executeCMDinDiagBMCOS "show Counters all" "${sw_ip}"
    return 0
}
...
if [ ! -d "${LOGFOLDER}/TG_Snake_Counter/PASS" ];then
    mkdir -p "${LOGFOLDER}/TG_Snake_Counter/PASS"
    mkdir -p "${LOGFOLDER}/TG_Snake_Counter/FAIL"
fi
...
do
    ...
    if [ "${PIPESTATUS[0]}" -ne "0" ];then
        if [ "$cycle" -lt "$retry" ];then
            ...
            file_name="${LOGFOLDER}/TG_Snake_Counter/FAIL/${SN}"_"Megazord"_"MP2"-${index}_${sw_sn}_TG_Snake_Counter_FAIL_"${time}".log
        ...
        show_end_msg | tee -a ${LOGFILE}
        time=$(date "+%F_%T" | sed -e "s\:\-\g")
        file_name="${LOGFOLDER}/TG_Snake_Counter/PASS/${SN}"_"Megazord"_"MP2"-${index}_${sw_sn}_TG_Snake_Counter_PASS_"${time}".log
        cp -f ${LOGFILE} ${file_name}
        echo "Log file : ${file_name}"
